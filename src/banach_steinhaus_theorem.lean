...
-- In the proof I needed to deal with balls in metric spaces, personally I
-- prefer using the notation Bᵣ(x) to denote a ball of radius r centered at x.
-- because of this I decided to introduce the following notation:
...
∃ (x₀ : X) (r : ℝ), 0 < r ∧  ∃ K' ≥ 0, ∀ (i : ι) x ∈ B_{r}(x₀), ‖f i x‖ ≤ K' :=
begin
  ...
  have hA_closed : ∀ n : ℕ, is_closed (A n),
    from λ n, is_closed_Inter (λ i, is_closed_le (continuous.norm (h_cont i)) continuous_const),
  ...
  have hBall : ∃ δ > 0, (B_{δ}(x₀)) ⊆ interior (A k₀),
...
begin
  ...
  { -- Here we assume x ≠ 0.
    ...
    -- Both of these can be derived from the fact that 0 < (2 * ‖x‖) / r
    -- so we prove just that one.
    have h_pos : 0 < (2 * ‖x‖) / r,
      from div_pos (mul_pos two_pos (norm_pos_iff.mpr h)) hr,
    ...
    { nth_rewrite 0 hx,
      nth_rewrite 1 ← abs_eq_self.mpr (le_of_lt h_pos),
      conv { to_lhs, find (r / _) { rw ← inv_div, }},
      -- Assumption that (2 * ‖x‖) / r ≠ 0 is derived from h_pos
      exact linear_manipulation (f i) x₀ x (ne_comm.mp (ne_of_lt h_pos)), },
